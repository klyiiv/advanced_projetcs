...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        res['response']['text'] = f'Привет! Купи {tovar}!'
        # Получим подсказки
    ...
    m = ['ладно',
         ...
         'покупаю',
         'хорошо']
    if any(i in str(req['request']['original_utterance'].lower()) for i in m):
        # Пользователь согласился, прощаемся.
        res['response']['text'] = f'{tovar.capitalize()} можно найти на Яндекс.Маркете!'
        if tovar == "кролика":
    ...
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}', а ты купи {tovar}!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={tovar[:-1]}",
            "hide": True
        })
